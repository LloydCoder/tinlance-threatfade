...
Capture adapters feed canonical events through the existing detection engine
adapter and into the bounded durable queue. The runtime continues collecting
while the sender/control plane is unavailable and shuts down gracefully.
"""
...
class EdgeSensorRuntime:
    def __init__(self, *, sensor_id: str, tenant_id: str, registry: SensorRegistry,
                 ...
                 detection_pipeline=None, limits: RuntimeLimits = RuntimeLimits()):
    ...
    def ingest(self, event: SignalEvent) -> bool:
        ...
        if self.detection_pipeline is not None:
            result = self.detection_pipeline.ingest(event)
    ...
    def health(self) -> dict:
        return {"sensor_id": self.sensor_id, "tenant_id": self.tenant_id,
                ...
                "detections": self._detections, "sender_configured": self.sender is not None,

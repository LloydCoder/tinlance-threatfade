...
GitHub artifact attestations bind artifacts to the workflow, repository, commit and build environment and can include SBOM attestations. GitHub documents these attestations as a supply-chain integrity mechanism and notes that artifact attestations provide SLSA Build Level 2; reusable workflows can be used to move toward Build Level 3. citeturn0search1turn0search3

SLSA 1.2 defines provenance as verifiable information that tracks an artifact to the source and build process that produced it. citeturn0search14
...
Docker recommends digest-pinning base images because tags are mutable and do not guarantee that future builds consume the same bytes. citeturn3search4 Kubernetes likewise documents digests as immutable image identifiers while tags can move. citeturn5search1
...
Trivy supports configuration-file driven scanner settings and Kubernetes/Dockerfile misconfiguration scanning. citeturn12search0turn12search1
...
GitHub's current `actions/attest` supports provenance and SBOM modes and requires `id-token`, `attestations`, and appropriate artifact permissions. citeturn7search0turn7search2
...
Kubernetes documents that without NetworkPolicies, pods are otherwise allowed ingress and egress by default; default-deny policies are therefore an important isolation primitive. citeturn5search7

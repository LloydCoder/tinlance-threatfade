...
1. **Primary continuity:** provider-native PostgreSQL PITR/WAL archiving. PostgreSQL documents continuous archiving as the mechanism for point-in-time recovery; the continuous WAL sequence must cover the base backup.
2. **Portable recovery:** daily custom-format logical dump produced by `scripts/backup.py`. PostgreSQL documents the custom format as a flexible archive designed for `pg_restore`.
...
- A restore from an untrusted archive must be treated as code execution against the restore environment.
...
5. If PITR is required, restore the physical base backup and replay archived WAL to the recovery target; `pg_dump` is not a WAL replay base.
...
<!-- workflow verification marker: 2026-08-22T10:50Z -->

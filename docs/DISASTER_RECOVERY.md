...
The RPO/RTO values are engineering targets, not contractual guarantees. Actual production values depend on the selected PostgreSQL provider, WAL/PITR configuration, storage durability and incident scope.

## Backup tiers

1. **Primary continuity:** provider-native PostgreSQL PITR/WAL archiving. PostgreSQL documents continuous archiving as the mechanism for point-in-time recovery; a continuous WAL sequence must cover the base backup. urlPostgreSQL continuous archiving and PITR documentationhttps://www.postgresql.org/docs/current/continuous-archiving.html
...
4. **Restore drill:** restore into an isolated database, apply/verify Alembic migration state, run integrity checks, and only then mark the backup usable.
...
- Credentials used for backup/restore must be short-lived or scoped to the minimum required database privileges.
...
- A restore from an untrusted archive must be treated as code execution against the restore environment; PostgreSQL explicitly warns that restores can execute arbitrary code from the source archive. urlPostgreSQL pg_dump security warninghttps://www.postgresql.org/docs/current/app-pgdump.html
...
4. Restore the logical archive into the isolated target using `pg_restore` with a least-privileged restore workflow.
5. If PITR is required, restore the physical base backup and replay archived WAL to the recovery target; do not attempt PITR from `pg_dump`, because logical dumps are not WAL replay bases. urlPostgreSQL PITR documentationhttps://www.postgresql.org/docs/current/continuous-archiving.html
...
The repository can automate backup creation and artifact verification. Provider-specific PITR/WAL configuration, cross-region replication, object-storage encryption/KMS, DNS failover and production promotion remain deployment-environment responsibilities and must be configured in the chosen infrastructure platform.

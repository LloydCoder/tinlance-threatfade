...
def test_owner_admin_analyst_viewer_rbac_and_cross_tenant_isolation():
    ...
    accepted = accept_invitation(analyst.subject, token, analyst.email)
    assert accepted == org_a.id
    assert membership(analyst.subject, org_a.id).role == "analyst"
    assert membership(analyst.subject, org_b.id) is None
    with pytest.raises(PermissionError):
        change_member_role(analyst.subject, org_b.id, other.subject, "viewer")
    members = list_members(owner.subject, org_a.id)
    assert {member["role"] for member in members} == {"owner", "analyst"}
...
def test_invitation_is_single_use_and_email_bound():
    ...
    with pytest.raises(PermissionError):
        accept_invitation(invited.subject, token2, invited.email)
...
def test_session_expiry_is_enforced():
    ...
    session_token = register_session(user.subject, None, "pytest", "127.0.0.1")
    # The public registration API enforces a minimum lifetime; validation remains
    # server-side and the returned expiry is always authoritative.

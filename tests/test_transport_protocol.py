...
def test_signed_batch_is_verified_before_replay_acceptance(tmp_path: Path):
    ...
    ledger = DurableReplayLedger(tmp_path / "replay.db")
    records = [
        {"sequence_no": 1, "payload": make_event("e1").canonical_bytes()},
        {"sequence_no": 2, "payload": make_event("e2").canonical_bytes()},
    ]
    batch = sign_batch(records, tenant_id="t", sensor_id="s", batch_id="b1", signer=signer)
...
def test_signed_batch_tenant_and_signature_fail_closed(tmp_path: Path):
    signer = EvidenceSigner()
    ledger = DurableReplayLedger(tmp_path / "replay.db")
    records = [{"sequence_no": 1, "payload": make_event("e1").canonical_bytes()}]
    batch = sign_batch(records, tenant_id="t", sensor_id="s", batch_id="b1", signer=signer)
    ...
    tampered = replace(batch, first_sequence=99)
    with pytest.raises(ValueError):
        verify_and_accept_batch(tampered, trusted_key=signer.metadata, replay_ledger=ledger, expected_tenant="t", expected_sensor="s")


def test_trust_store_rotation_and_revocation(tmp_path: Path):
    store = SigningTrustStore(tmp_path / "keys.db")
    first = EvidenceSigner().metadata
    second = EvidenceSigner().metadata
    store.add(first)
    store.add(second)
    assert store.get(first.key_id) is not None
    store.revoke(first.key_id)
    assert store.get(first.key_id).revoked_at is not None
    assert second.key_id in {key.key_id for key in store.active_keys()}
    assert first.key_id not in {key.key_id for key in store.active_keys()}

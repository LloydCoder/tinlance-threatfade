...
def _guard(request: Request, x_api_key: Optional[str], permission: str):
    ...
    AUDIT.record("authorization", principal, request, {"permission": permission})
...
def _build_response(result, mitre_ttp, source_name, export_format, principal, tenant_id):
    ...
    AUDIT.record("detection.created", principal, metadata={"detection_id": detection_id, "source": source_name})
    return DetectionResponse(
        timestamp=datetime.now(timezone.utc).isoformat(), detection_id=detection_id, tenant_id=tenant_id,
        detected=bool(result.get("detected", False)), confidence=result.get("confidence", "info"),
...
def _byte_entropy(data: bytes) -> float:
    ...
    for count in freq:
        if count:
            p = count / len(data); ent -= p * math.log2(p)
    return ent
...
def _pcap_to_signals(pcap_path: str, interval_sec: int = 5):
    ...
    except Exception as exc: raise HTTPException(status_code=400, detail="Invalid or unreadable PCAP") from exc
    events = []
    for pkt in packets:
    ...
    if not events: return list(range(20)), [0.5] * 20
    events.sort(key=lambda item: item["time"]); start, end = int(events[0]["time"]), int(events[-1]["time"]); duration = end - start
    ...
    while current <= end:
        ...
        if not bucket: signal = 0.0
        else:
            count_score = min(1.0, len(bucket) / 100.0); total_bytes = sum(p["size"] for p in bucket); byte_score = min(1.0, total_bytes / 50000.0)
            payload_ratio = sum(p["payload_size"] for p in bucket) / total_bytes if total_bytes else 0.0
            entropies = [p["raw_entropy"] for p in bucket if p["raw_entropy"] is not None]; entropy_score = (sum(entropies) / len(entropies)) / 8.0 if entropies else 0.0
            raw_ratio = len(entropies) / len(bucket); signal = (0.5 * entropy_score + 0.2 * count_score + 0.2 * byte_score + 0.1 * payload_ratio) if raw_ratio >= 0.5 else (0.1 * entropy_score + 0.4 * count_score + 0.3 * byte_score + 0.2 * payload_ratio)
        timestamps.append(current - start); values.append(float(signal)); current += interval_sec
    while len(values) < 12: values.append(0.0); timestamps.append((timestamps[-1] + interval_sec) if timestamps else len(timestamps))
    lo, hi = min(values), max(values)
    if hi > lo: values = [(v - lo) / (hi - lo) for v in values]
...
@app.get("/health")
def health(): return {"status": "ok", "tool": "ThreatFade", "version": CONFIG["branding"]["version"], "company": CONFIG["branding"]["company"], "timestamp": datetime.now(timezone.utc).isoformat()}

@app.get("/ready")
def readiness():
    checks = {"config": bool(CONFIG), "dashboard": os.path.exists("dashboard/index.html"), "storage": True}
    ready = all(checks.values())
    if not ready: raise HTTPException(status_code=503, detail={"status": "not_ready", "checks": checks})
    return {"status": "ready", "checks": checks, "version": CONFIG["branding"]["version"], "slo": slo_targets()}

@app.get("/version")
def version(): return {"name": CONFIG["branding"]["name"], "version": CONFIG["branding"]["version"], "company": CONFIG["branding"]["company"], "license": "Apache 2.0 (open-core)"}
...
def auth_config():
    return {"oidc_required_in_production": True, "issuer_configured": bool(os.getenv("THREATFADE_OIDC_ISSUER")), "rbac_roles": ["viewer", "analyst", "api_only", "admin", "tenant_admin"]}
...
def detect(req: DetectRequest, request: Request, x_api_key: Optional[str] = Header(default=None)):
    ...
    if len(req.values) < 12: raise HTTPException(status_code=400, detail=f"Need at least 12 signal values, got {len(req.values)}")
    if any(not math.isfinite(v) for v in req.values): raise HTTPException(status_code=400, detail="Signal values must be finite numbers")
    if req.timestamps is not None:
        if len(req.timestamps) != len(req.values): raise HTTPException(status_code=400, detail="timestamps length must match values length")
        if any(not math.isfinite(v) for v in req.timestamps): raise HTTPException(status_code=400, detail="timestamps must be finite numbers")
    timestamps = req.timestamps or list(range(len(req.values))); result = _run_detection(timestamps, req.values, req.use_ml)
...
async def detect_pcap(request: Request, file: UploadFile = File(...), use_ml: bool = False, export_format: Optional[str] = None, x_api_key: Optional[str] = Header(default=None)):
    principal, tenant_id = _guard(request, x_api_key, "detection:run")
    validate_pcap_upload(file); content = await read_limited_upload(file)
    ...
    try: timestamps, values = _pcap_to_signals(tmp_path); result = _run_detection(timestamps, values, use_ml)
    finally:
...
def detect_scenario(req: ScenarioRequest, request: Request, x_api_key: Optional[str] = Header(default=None)):
    principal, tenant_id = _guard(request, x_api_key, "detection:run"); tenant_id = require_tenant(principal, req.tenant_id)
    valid = ["c2_quieting", "lotl_gradual", "gnss_jam", "normal_with_fade", "mixed"]
    if req.scenario not in valid: raise HTTPException(status_code=400, detail=f"Invalid scenario. Choose from: {valid}")
    timestamps, values = generate_signals(req.scenario); result = _run_detection(timestamps, values, req.use_ml)

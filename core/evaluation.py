...
The evaluator separates ground truth from detector output and reports metrics
that remain meaningful when a corpus is imbalanced. It is intentionally free
of ML-framework dependencies so the evaluation contract can be used by unit
benchmarks, real-PCAP adapters, and future purple-team runners.
"""
...
def _metric_from_flags(expected: np.ndarray, detected: np.ndarray, metric: str) -> float:
    cases = [
        EvaluationCase(str(i), "bootstrap", bool(expected[i]), bool(detected[i]))
...
def bootstrap_confidence_interval(
    ...
    """Return a deterministic percentile bootstrap interval for a metric."""
...
def evaluate_cases(cases: Iterable[EvaluationCase], bootstrap: bool = True) -> dict:
    ...
    if bootstrap and materialized:
        result["confidence_intervals"] = {
            metric: bootstrap_confidence_interval(materialized, metric=metric)

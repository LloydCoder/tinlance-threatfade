...
def inbox_total(tenant_id: str, *, status: str | None = None, assignee: str | None = None) -> int:
    ...
    with Session(ENGINE) as s:
        ...
        if status: q = q.join(DetectionWorkflowRecord, (DetectionWorkflowRecord.detection_id == DetectionRecord.id) & (DetectionWorkflowRecord.tenant_id == tenant_id)).where(DetectionWorkflowRecord.status == status)
        if assignee: q = q.join(DetectionWorkflowRecord, (DetectionWorkflowRecord.detection_id == DetectionRecord.id) & (DetectionWorkflowRecord.tenant_id == tenant_id)).where(DetectionWorkflowRecord.assignee == assignee)
...
def investigation(tenant_id: str, detection_id: int) -> dict[str, Any] | None:
    with Session(ENGINE) as s:
        ...
        return {
            "detection": {"id": d.id, "subject": d.subject, "source": d.source, "confidence": d.confidence, "score": float(d.score), "mitre_ttp": d.mitre_ttp, "correlation_id": d.correlation_id, "created_at": d.created_at.isoformat(), "evidence": _json(d.evidence_json), "provenance": {"input_sha256": d.input_sha256, "rule_pack_sha256": d.rule_pack_sha256, "engine_version": d.engine_version, "model_sha256": d.model_sha256, "config_sha256": d.config_sha256}},
            "workflow": {"status": workflow.status if workflow else "new", "assignee": workflow.assignee if workflow else None, "priority": workflow.priority if workflow else 50},
            "evidence": [{"id": x.id, "type": x.evidence_type, "media_type": x.media_type, "hash": x.content_sha256, "size_bytes": x.size_bytes, "collected_at": x.collected_at.isoformat(), "source_uri": x.source_uri} for x in evidence],
            "entities": [{"id": x.id, "entity_type": x.entity_type, "entity_key": x.entity_key, "attributes": _json(x.attributes_json), "created_at": x.created_at.isoformat()} for x in entities],
            "sessions": [{"id": x.id, "session_key": x.session_key, "protocol": x.protocol, "started_at": x.started_at.isoformat() if x.started_at else None, "ended_at": x.ended_at.isoformat() if x.ended_at else None, "attributes": _json(x.attributes_json)} for x in sessions],
            "dispositions": [{"id": x.id, "reason": x.reason, "note": x.note, "analyst": x.analyst, "case_id": x.case_id, "created_at": x.created_at.isoformat()} for x in dispositions],
            "cases": [{"id": x.id, "title": x.title, "status": x.status, "owner": x.owner, "created_at": x.created_at.isoformat()} for x in cases],
        }

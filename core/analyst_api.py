...
from core.analyst_workflow import DISPOSITION_REASONS, STATUSES, get_investigation, list_inbox, timeline, update_workflow
from core.analyst_workflow_pg import add_disposition, create_case
...
def _triage_severity(score: float, confidence: str) -> str:
    ...
    if confidence.lower() in {"critical", "high"} or score >= 0.9: return "high"
    if confidence.lower() in {"medium", "moderate"} or score >= 0.6: return "medium"
    return "low"

@router.get("/inbox")
def analyst_inbox(request: Request, status: Optional[str] = Query(default=None), limit: int = Query(default=50, ge=1, le=200), offset: int = Query(default=0, ge=0, le=100000), sort: str = Query(default="created_at"), order: str = Query(default="desc"), x_api_key: Optional[str] = Header(default=None)):
    _, tenant_id = _guard(request, x_api_key, "detection:read")
    if status is not None and status not in STATUSES: raise HTTPException(400, "Invalid workflow status")
    if sort not in {"created_at", "score", "priority", "id"}: raise HTTPException(400, "Invalid sort field")
    if order.lower() not in {"asc", "desc"}: raise HTTPException(400, "Invalid sort order")
    ...
    for item in items:
        ...
        if hasattr(item["created_at"], "isoformat"): item["created_at"] = item["created_at"].isoformat()
...
def analyst_detection(detection_id: int, request: Request, x_api_key: Optional[str] = Header(default=None)):
    _, tenant_id = _guard(request, x_api_key, "detection:read")
    if detection_id < 1: raise HTTPException(400, "Invalid detection id")
    data = get_investigation(tenant_id, detection_id)
    if data is None: raise HTTPException(404, "Detection not found")
...
def analyst_timeline(detection_id: int, request: Request, x_api_key: Optional[str] = Header(default=None)):
    _, tenant_id = _guard(request, x_api_key, "detection:read")
    if detection_id < 1: raise HTTPException(400, "Invalid detection id")
    if get_investigation(tenant_id, detection_id) is None: raise HTTPException(404, "Detection not found")
...
def analyst_workflow(detection_id: int, payload: WorkflowPatch, request: Request, x_api_key: Optional[str] = Header(default=None)):
    ...
    if detection_id < 1: raise HTTPException(400, "Invalid detection id")
    if payload.status is None and payload.assignee is None and payload.priority is None: raise HTTPException(400, "At least one workflow field is required")
    try: update_workflow(tenant_id, detection_id, principal.subject, status=payload.status, assignee=payload.assignee, priority=payload.priority)
...
def analyst_disposition(detection_id: int, payload: DispositionRequest, request: Request, x_api_key: Optional[str] = Header(default=None)):
    principal, tenant_id = _guard(request, x_api_key, "case:write")
    if detection_id < 1 or payload.reason not in DISPOSITION_REASONS: raise HTTPException(400, "Invalid disposition")
    try: disposition_id = add_disposition(tenant_id, detection_id, principal.subject, payload.reason, payload.note)
    except ValueError as exc: raise HTTPException(400, str(exc)) from exc
    if disposition_id == 0: raise HTTPException(404, "Detection not found")
...
def analyst_create_case(detection_id: int, payload: CaseRequest, request: Request, x_api_key: Optional[str] = Header(default=None)):
    principal, tenant_id = _guard(request, x_api_key, "case:write")
    if detection_id < 1: raise HTTPException(400, "Invalid detection id")
    try: case = create_case(tenant_id, principal.subject, detection_id, payload.title)
    except ValueError as exc: raise HTTPException(400, str(exc)) from exc

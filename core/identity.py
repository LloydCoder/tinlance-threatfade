...
OIDC authenticates the human identity. This module owns application authorization
state: organizations, memberships, invitations and server-side session revocation.
No browser-supplied role or tenant identifier is trusted without a membership check.
"""
...
ROLE_PERMISSIONS = {
    "owner": {"*"},
    "admin": {"detection:read", "detection:run", "case:read", "case:write", "org:read", "org:write", "member:write", "session:read", "session:write"},
    "analyst": {"detection:read", "detection:run", "case:read", "case:write", "session:read"},
    "viewer": {"detection:read", "case:read", "session:read"},
}
...
class MembershipRecord(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    __table_args__ = (UniqueConstraint("organization_id", "subject", name="uq_identity_membership_org_subject"),)
...
class AuthSessionRecord(Base):
    ...
    token_hash: Mapped[str] = mapped_column(String(64), unique=True, index=True, nullable=False)
    subject: Mapped[str] = mapped_column(String(255), index=True, nullable=False)
    active_organization_id: Mapped[str | None] = mapped_column(String(32), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    last_seen_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    revoked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
def ensure_user(subject: str, email: str | None = None, name: str | None = None) -> UserRecord:
    ...
    with Session(ENGINE) as session:
        ...
        if user is None:
            user = UserRecord(subject=subject, email=email, name=name, created_at=now, last_seen_at=now)
            session.add(user)
        else:
...
def get_user(subject: str) -> UserRecord | None:
    with Session(ENGINE) as session:
        return session.scalar(select(UserRecord).where(UserRecord.subject == subject))
...
def create_organization(subject: str, name: str, slug: str | None = None) -> OrganizationRecord:
    ...
    with Session(ENGINE) as session:
        if session.scalar(select(OrganizationRecord).where(OrganizationRecord.slug == normalized)):
            raise ValueError("organization slug is already in use")
        org = OrganizationRecord(id=uuid.uuid4().hex, slug=normalized, name=name, created_by=subject, created_at=_now())
        session.add(org)
        session.add(MembershipRecord(organization_id=org.id, subject=subject, role="owner", created_at=_now()))
        session.commit()
        session.refresh(org)
        return org
...
def require_permission(subject: str, organization_id: str, permission: str) -> MembershipRecord:
    ...
    if row is None or permission not in ROLE_PERMISSIONS.get(row.role, set()) and "*" not in ROLE_PERMISSIONS.get(row.role, set()):
        raise PermissionError("insufficient organization permissions")
    return row
...
def accept_invitation(subject: str, token: str, email: str | None) -> str:
    ...
    with Session(ENGINE) as session:
        invitation = session.scalar(select(InvitationRecord).where(InvitationRecord.token_hash == _hash(token)))
        now = _now()
        ...
        if existing:
            ...
            session.add(MembershipRecord(organization_id=invitation.organization_id, subject=subject, role=invitation.role, created_at=now))
...
def change_member_role(actor: str, organization_id: str, target_subject: str, role: str) -> None:
    require_permission(actor, organization_id, "member:write")
    if role not in ROLES - {"owner"}:
    ...
    with Session(ENGINE) as session:
        ...
        if target.role == "owner":
            raise PermissionError("organization owner role cannot be reassigned")
        target.role = role
        session.commit()
...
def remove_member(actor: str, organization_id: str, target_subject: str) -> None:
    ...
    with Session(ENGINE) as session:
        ...
        if target is None:
            raise ValueError("member not found")
        if target.role == "owner":
            raise PermissionError("organization owner cannot be removed")
...
def revoke_all_sessions(subject: str) -> None:
    with Session(ENGINE) as session:
        ...
        rows = session.scalars(select(AuthSessionRecord).where(AuthSessionRecord.subject == subject, AuthSessionRecord.revoked_at.is_(None)))
        for row in rows:
            row.revoked_at = now
        session.commit()
...
def switch_session_organization(subject: str, token: str, organization_id: str) -> None:
    ...
    with Session(ENGINE) as session:
        ...
        row.active_organization_id = organization_id

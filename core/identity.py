...
def create_organization(subject: str, name: str, slug: str | None = None) -> OrganizationRecord:
    ...
    with Session(ENGINE) as session:
        ...
        session.add(org); session.add(MembershipRecord(organization_id=org.id, subject=subject, role="owner", created_at=_now())); session.commit(); session.refresh(org); return org

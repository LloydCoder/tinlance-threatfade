...
_SINGLE_INDEXES: dict[str, dict[str, str]] = {
    "detections": {"tenant_id": "ix_detections_tenant_id", "correlation_id": "ix_detections_correlation_id", "input_sha256": "ix_detections_input_sha256", "rule_pack_sha256": "ix_detections_rule_pack_sha256", "engine_version": "ix_detections_engine_version", "model_sha256": "ix_detections_model_sha256", "config_sha256": "ix_detections_config_sha256"},
    "cases": {"tenant_id": "ix_cases_tenant_id"},
    "detection_feedback": {"tenant_id": "ix_detection_feedback_tenant_id", "detection_id": "ix_detection_feedback_detection_id"},
    "case_comments": {"tenant_id": "ix_case_comments_tenant_id", "case_id": "ix_case_comments_case_id"},
    "case_events": {"tenant_id": "ix_case_events_tenant_id", "case_id": "ix_case_events_case_id", "correlation_id": "ix_case_events_correlation_id"},
    "tenant_config": {"tenant_id": "ix_tenant_config_tenant_id"},
    "audit_events": {"tenant_id": "ix_audit_events_tenant_id", "request_id": "ix_audit_events_request_id", "correlation_id": "ix_audit_events_correlation_id"},
    "evidence": {"tenant_id": "ix_evidence_tenant_id", "correlation_id": "ix_evidence_correlation_id", "case_id": "ix_evidence_case_id", "content_sha256": "ix_evidence_content_sha256"},
    "provenance": {"tenant_id": "ix_provenance_tenant_id", "correlation_id": "ix_provenance_correlation_id", "detection_id": "ix_provenance_detection_id"},
    "investigation_timeline": {"tenant_id": "ix_investigation_timeline_tenant_id", "case_id": "ix_investigation_timeline_case_id", "correlation_id": "ix_investigation_timeline_correlation_id"},
    "legal_holds": {"tenant_id": "ix_legal_holds_tenant_id"},
    "detection_workflow": {"tenant_id": "ix_detection_workflow_tenant", "detection_id": "ix_detection_workflow_detection"},
    "case_detection_links": {"tenant_id": "ix_case_detection_links_tenant", "case_id": "ix_case_detection_links_case", "detection_id": "ix_case_detection_links_detection"},
    "analyst_dispositions": {"tenant_id": "ix_analyst_dispositions_tenant", "detection_id": "ix_analyst_dispositions_detection", "case_id": "ix_analyst_dispositions_case"},
    "investigation_entities": {"tenant_id": "ix_investigation_entities_tenant", "correlation_id": "ix_investigation_entities_correlation"},
    "investigation_sessions": {"tenant_id": "ix_investigation_sessions_tenant", "session_key": "ix_investigation_sessions_key", "correlation_id": "ix_investigation_sessions_correlation"},
    "identity_users": {"subject": "ix_identity_users_subject", "email": "ix_identity_users_email"},
    "identity_organizations": {"slug": "ix_identity_organizations_slug"},
    "identity_memberships": {"organization_id": "ix_identity_memberships_org", "subject": "ix_identity_memberships_subject"},
    "identity_invitations": {"organization_id": "ix_identity_invitations_org", "email": "ix_identity_invitations_email", "token_hash": "ix_identity_invitations_token_hash"},
    "identity_sessions": {"token_hash": "ix_identity_sessions_token_hash", "subject": "ix_identity_sessions_subject"},
}
...
def _reconcile_unique_constraint(table, columns: tuple[str, ...], canonical_name: str) -> None:
    for constraint in table.constraints:
        if isinstance(constraint, UniqueConstraint) and tuple(column.name for column in constraint.columns) == columns:
            ...
            return
    unique_index = next((index for index in table.indexes if index.unique and _columns_key(index) == columns), None)
...
def reconcile_metadata(metadata: MetaData | None = None) -> None:
    ...
    for table_name, canonical_name, columns in _COMPOSITE_INDEXES:
        _ensure_index(metadata.tables[table_name], columns, canonical_name)
    for (table_name, columns), canonical_name in _UNIQUE_CONSTRAINTS.items():
        _reconcile_unique_constraint(metadata.tables[table_name], columns, canonical_name)
    for table_name, columns in _UNIQUE_CONSTRAINT_WITH_INDEX:
        _ensure_index(metadata.tables[table_name], columns, _SINGLE_INDEXES[table_name][columns[0]])

...
class DurableEventQueue:
    ...
    def enqueue(self, event: SignalEvent, *, priority: int = 50) -> int:
        ...
        if len(payload) > MAX_EVENT_BYTES: raise ValueError("event exceeds maximum size")
        digest = event.digest(); self._db.execute("BEGIN IMMEDIATE")
        try:
...
class BandwidthPolicy:
    bytes_per_second: int = 256 * 1024; burst_bytes: int = 1024 * 1024; max_batch_bytes: int = 512 * 1024
...
class BandwidthAwareTransmitter:
    ...
    def select(self, records: Sequence[Mapping[str, Any]]) -> list[Mapping[str, Any]]:
        now = _now(); self._tokens = self.policy.allowance(now - self._last, self._tokens); self._last = now; selected: list[Mapping[str, Any]] = []; used = 0
...
def build_evidence_package(*, tenant_id: str, sensor_id: str, events: Sequence[SignalEvent], evidence: Sequence[Mapping[str, Any]] = (), provenance: Mapping[str, Any] | None = None, signer: EvidenceSigner) -> bytes:
    ...
    signed = _canonical(manifest); manifest["signature"] = signer.sign(signed); manifest["manifest_sha256"] = _sha256(signed); out = BytesIO()
...
def verify_evidence_package(package: bytes, trusted_keys: Mapping[str, SigningKey], *, tenant_id: str | None = None, at: datetime | None = None) -> dict[str, Any]:
    ...
    if any(_event_digest(record) != record.get("event_digest") for record in events): raise ValueError("event content digest mismatch")
